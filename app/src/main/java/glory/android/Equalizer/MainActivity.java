package glory.android.Equalizer;
...
import static com.pesonal.adsdk.AppManage.app_innerClickCntSwAd;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //loadInterstitialAd
        AppManage.getInstance(this).loadInterstitialAd(this, AppManage.ADMOB_I[0], AppManage.FACEBOOK_I[0]);
        //banner ads
        AppManage.getInstance(this).showNativeBanner(linearLayout, AppManage.ADMOB_I[0], AppManage.FACEBOOK_I[0]);
        AppManage.getInstance(this).showNativeBanner(linearLayout);

        loadEqualizerSettings();
        mediaPlayer = MediaPlayer.create(MainActivity.this, R.raw.lenka);
        final int sessionId = mediaPlayer.getAudioSessionId();
        mediaPlayer.setLooping(true);
        EqualizerFragment equalizerFragment = EqualizerFragment.newBuilder().setAccentColor(Color.parseColor("#4caf50")).setAudioSessionId(sessionId).build();
    ...
    private void showInDialog() {
        ...
        if (sessionId > 0) {
            DialogEqualizerFragment fragment = DialogEqualizerFragment.newBuilder()
                    .setAudioSessionId(sessionId).title(R.string.app_name)
                    .themeColor(ContextCompat.getColor(this, R.color.primaryColor))
                    .textColor(ContextCompat.getColor(this, R.color.textColor)).accentAlpha(ContextCompat.getColor(this, R.color.playingCardColor)).darkColor(ContextCompat.getColor(this, R.color.primaryDarkColor)).setAccentColor(ContextCompat.getColor(this, R.color.secondaryColor)).build();
    ...
    protected void onResume() {
        super.onResume();
        try {
            new Handler().postDelayed(() -> mediaPlayer.start(), 2000);
        } catch (Exception ex) {
            //ignore
        }
    }
